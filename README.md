...
- Open preview in a separate tab for PDF export
- Theme customization with 15 built-in presets
- Render mode and theme persisted to localStorage
- Accessible: focus management, ARIA labels, keyboard navigation, screen reader support
...
6. Click "Copy Preview" in the header to copy the rendered content (including diagrams as images) to your clipboard, then paste into Outlook, Word, or Confluence
7. Use the strategy dropdown next to Copy Preview to choose a diagram conversion method (Auto, SVG, or DOM)
8. Click "Open Preview" to open a standalone preview for PDF export via browser print
...
3. Multiple diagrams are converted in parallel for faster processing
4. Critical CSS (fonts, colors, borders, table styling) is inlined since clipboard HTML has no stylesheet
5. The result is written to the clipboard as both `text/html` (for rich paste) and `text/plain` (for plain text editors) using `navigator.clipboard.write()`
